...
class TestAlertsTrader(unittest.TestCase):
    def tracker_order(self, brokerage):
        ...
        expected["PnL-alert"] = (
            100 * (expected["STC1-Price-alert"] - expected["Price-alert"]) / expected["Price-alert"]
        )
        expected["PnL$-alert"] = (
            expected["PnL-alert"] * expected["STC1-Qty"] * expected["Price-alert"]
        )
        expected["PnL-actual"] = (
            100
            * (expected["STC1-Price-actual"] - expected["Price-actual"])
            / expected["Price-actual"]
        )
        expected["PnL$-actual"] = (
            expected["PnL-actual"] * expected["STC1-Qty"] * expected["Price-actual"]
        )
        ...
        brokerage.send_order.return_value = [expected["BTO-Status"], expected["ordID"]]
        ...
        # make STC order
        message.content = (
            f"STC {expected['STC1-Qty']} AI 25c {expdate} @ {expected['STC1-Price-alert']}"
        )
        pars, order = parse_trade_alert(message.content)
        order["Trader"] = f"{message.author.name}#{message.author.discriminator}"
        order["Date"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
        ...
        # Generate return vals for the brokerage
        brokerage.get_quotes.return_value = {
            f"AI_{expdate.replace('/', '')}23C25": {"bidPrice": expected["STC1-Price-actual"]}
        }
        brokerage.send_order.return_value = [expected["STC1-Status"], expected["STC1-ordID"]]
        brokerage.get_order_info.return_value = [
    ...
    def tracker_order_sto(self, brokerage):
        ...
        exit_plan["PT1"] = round(
            expected["Price"] * (1 - float(cfg["shorting"]["BTC_PT"]) / 100), 2
        )
        ...
        expected["PnL$"] = round(expected["PnL"] * expected["STC1-Qty"] * expected["Price"], 1)
        expected["PnL-alert"] = (
            100 * (expected["Price-alert"] - expected["STC1-Price-alert"]) / expected["Price-alert"]
        )
        expected["PnL$-alert"] = (
            expected["PnL-alert"] * expected["STC1-Qty"] * expected["Price-alert"]
        )
        expected["PnL-actual"] = (
            100
            * (expected["Price-actual"] - expected["STC1-Price-actual"])
            / expected["Price-actual"]
        )
        expected["PnL$-actual"] = (
            expected["PnL-actual"] * expected["STC1-Qty"] * expected["Price-actual"]
        ...
        brokerage.get_order_info.return_value = [
            expected["BTO-Status"],
            {
                ...
                "status": expected["BTO-Status"],
                "orderLegCollection": [
                    {"instrument": {"symbol": symbol}, "instruction": "SELL_TO_OPEN"},
                ],
            },
        ]
        ...
        # make STC order
        message.content = (
            f"BTC {expected['STC1-Qty']} AI 25c {expdate} @ {expected['STC1-Price-alert']}"
        )
        pars, order = parse_trade_alert(message.content)
        order["Trader"] = f"{message.author.name}#{message.author.discriminator}"
        order["Date"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
        ...
        # Generate return vals for the brokerage
        brokerage.get_quotes.return_value = {
            f"AI_{expdate.replace('/', '')}23C25": {"askPrice": expected["STC1-Price-actual"]}
        }
        brokerage.send_order.return_value = [expected["STC1-Status"], expected["STC1-ordID"]]
        brokerage.get_order_info.return_value = [
            expected["STC1-Status"],
