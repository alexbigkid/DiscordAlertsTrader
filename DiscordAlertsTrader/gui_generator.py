...
def max_dig_len(values, decim=2):
    ...
    values_s = [
        str(round(v, decim)) if v % 1 else str(round(v))
        for v in values
...
def get_portf_data(
    ...
    **kwargs,
    ...
    for price in ["Price", "Price-alert", "Price-actual"]:
        data[f"STC-{price.replace('Current', 'actual')}"] = data.apply(
            calculate_weighted_mean, args=(price,), axis=1
        )
    ...
    data["STC-Prices-actual"] = data[
        ["STC1-Price-actual", "STC2-Price-actual", "STC3-Price-actual"]
    ].apply(lambda x: "/".join(x.astype(str)).replace("/nan", ""), axis=1)
    data["STC-Prices-alert"] = data[
        ["STC1-Price-alert", "STC2-Price-alert", "STC3-Price-alert"]
    ].apply(lambda x: "/".join(x.astype(str)).replace("/nan", ""), axis=1)
    ...
    if len(data):
        sumtotal = dict.fromkeys(data.columns, "")
        for sumcol in ["PnL", "PnL-alert", "PnL-actual"]:
            sumtotal[sumcol] = (
                f"{data[sumcol].apply(lambda x: np.nan if x.strip() in ['', '-inf'] else eval(x)).mean():.2f}"
            )
        for sumcol in ["PnL$", "PnL$-alert", "PnL$-actual", "Qty", "filledQty"]:
            sumtotal[sumcol] = (
                f"{data[sumcol].apply(lambda x: np.nan if x.strip() in ['', '-inf'] else eval(x)).sum():.2f}"
            )
    ...
    **kwargs,
    ...
    if len(data):
        ...
        for sumcol in ["PnL", "PnL-actual"]:
            sumtotal[sumcol] = (
                f"{data[sumcol].apply(lambda x: np.nan if x == '' else eval(x)).mean():.2f}"
            )
        for sumcol in ["PnL$", "PnL$-actual"]:
            sumtotal[sumcol] = (
                f"{data[sumcol].apply(lambda x: np.nan if x == '' else eval(x)).sum():.2f}"
            )
        sumtotal["Date"] = data.iloc[len(data) - 1]["Date"]
        sumtotal["Symbol"] = "Total Average"
...
def get_stats_data(
    ...
    **kwargs,
    ...
    data["Date"] = data["Date"].apply(
        lambda x: datetime.strptime(x, "%Y-%m-%d %H:%M:%S.%f").strftime("%Y/%m/%d")
    )
    exclude["Open"] = True
    try:
        data = filter_data(
            ...
            stat_exc_chn,
            stat_exc_sym,
            max_trade_val=stat_max_trade_val,
            max_u_qty=stat_max_qty,
            max_dte=stat_dte_max,
            min_dte=stat_dte_min,
        )
    except Exception as e:
...
def get_live_quotes(portfolio, trader_port=False):
    ...
    for sym in quotes_sym:
        ...
        for ix, trade in trades.iterrows():
            ...
            order = {
                "Qty": trade["Qty"]
                if pd.isnull(trade.get("STC-Qty"))
                else trade["Qty"] - trade["STC-Qty"],
                "price": live_price,
...
def compute_live_trader_port(trade, order):
    ...
    if sold_tot == 0:
        return trade
    stc_PnL_all = (
        np.nansum([trade[f"STC{i}-PnL"] * trade[f"STC{i}-Qty"] for i in range(1, 4)]) / sold_tot
    )
    ...
    if trade["Type"] == "BTO":
        stc_PnL_all_alert = (
            np.nansum(
                [
                    (
                        float((trade[f"STC{i}-Price-alert"] - bto_price_alert) / bto_price_alert)
                        * 100
                    )
                    * trade[f"STC{i}-Qty"]
                    for i in range(1, 4)
        ...
        stc_PnL_all_curr = (
            np.nansum(
                [
                    (
                        float((trade[f"STC{i}-Price-actual"] - bto_price_actual) / bto_price_actual)
                        * 100
                    )
                    * trade[f"STC{i}-Qty"]
                    for i in range(1, 4)
        ...
        stc_PnL_all_alert = (
            np.nansum(
                [
                    (
                        float((bto_price_alert - trade[f"STC{i}-Price-alert"]) / bto_price_alert)
                        * 100
                    )
                    * trade[f"STC{i}-Qty"]
                    for i in range(1, 4)
        ...
        stc_PnL_all_curr = (
            np.nansum(
                [
                    (
                        float((bto_price_actual - trade[f"STC{i}-Price-actual"]) / bto_price_actual)
                        * 100
                    )
                    * trade[f"STC{i}-Qty"]
                    for i in range(1, 4)
...
def get_hist_msgs(
    ...
    **kwargs,
    ...
    data = pd.read_csv(
        op.join(cfg["general"]["data_dir"], f"{chan_name}_message_history.csv"),
...
def get_acc_bals(bksession):
    ...
    if acc_inf is None:
        acc_inf = bksession.get_account_info()
        if acc_inf is None:
            return {"id": 0, "balance": 0, "cash": 0, "funds": 0}, {
                "id": "00",
                "balance": 0,
                "cash": 0,
                "funds": 0,
    ...
    accnt = {
        "id": acc_inf["securitiesAccount"]["accountId"],
...
def order_info_pars(ord_dic, ord_list):
    ...
    for leg in ord_dic["orderLegCollection"]:
        ...
        sing_ord_c.insert(
            1, leg["instruction"].replace("BUY_TO_OPEN", "BUY").replace("SELL_TO_CLOSE", "SELL")
        )
...
def get_orders(acc_inf):
    if acc_inf.get("securitiesAccount") is None:
        return (
            ["NoAccount"],
            ["Sym", "Act", "Strat", "Price/stp", "Date", "Qty/fill", "Status", "ordId"],
            [],
        )
