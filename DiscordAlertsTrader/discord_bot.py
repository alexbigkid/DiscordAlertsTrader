...
class DiscordBot(discord.Client):
    def __init__(
        ...
        if (live_quotes and brokerage is not None and brokerage.name != "webull") or (
            brokerage is not None
            and brokerage.name == "webull"
        ...
        while self.live_quotes:
            ...
            for q in quote:
                ...
                with open(file_path, "a+") as f:
                    if do_header:
                        f.write("timestamp, quote, quote_ask\n")
    ...
    def load_data(self):
        ...
        for ch in self.channel_IDS.keys():
            ...
            if not os.path.exists(dt_fname):
                ...
                ch_dt.to_csv(
                    f"{self.cfg['general']['data_dir']}/{ch}_message_history_temp.csv", index=False
                )
            else:
    ...
    async def on_message(self, message):
        ...
        if message.channel.id == int(cfg["discord"]["commands_channel"]):
            ...
            and author.lower() not in split_strip(self.cfg["discord"]["authorwise_subscription"])
        ):
            return
        ...
        if pars is None:
            if self.chn_hist.get(chn) is not None:
                ...
                self.chn_hist[chn] = pd.concat(
                    [self.chn_hist[chn], msg.to_frame().transpose()], axis=0, ignore_index=True
                )
    ...
    def do_trade_alert(self, author, channel, order):
        ...
        ) or channel.lower() in split_strip(self.cfg["discord"]["channelwise_subscription"]):
            # ignore if no STC
            if (
        ...
        # in authors shorting list
            ...
            if (
                order["action"] == "BTC" and not self.cfg["shorting"].getboolean("DO_BTC_TRADES")
            ) or (
                order["action"] == "STO" and not self.cfg["shorting"].getboolean("DO_STO_TRADES")
            ):
                return False, order
