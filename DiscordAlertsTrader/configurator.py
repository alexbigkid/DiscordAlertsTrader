...
def update_port_cols():
    portfolio_newcols = {
        "Price-Current": "Price-actual",
        "PnL-Current": "PnL-actual",
        "$PnL-Current": "PnL$-actual",
        "$PnL": "PnL$",
        ...
        "$PnL-Alert": "PnL$-alert",
        "uQty": "Qty",
        "STC1-Alerted": "STC1-alerted",
        "STC1-uQty": "STC1-Qty",
        "STC1-Price-Alerted": "STC1-Price-alert",
        "STC1-Price-Current": "STC1-Price-actual",
        "STC2-Alerted": "STC2-alerted",
        "STC2-uQty": "STC2-Qty",
        "STC2-Price-Alerted": "STC2-Price-alert",
        "STC2-Price-Current": "STC2-Price-actual",
        "STC3-Alerted": "STC3-alerted",
        "STC3-uQty": "STC3-Qty",
        "STC3-Price-Alerted": "STC3-Price-alert",
    ...
    tracker_newcols = {
        "Amount": "Qty",
        "Price-current": "Price-actual",
        "Prices-current": "Prices-actual",
        "PnL-current": "PnL-actual",
        "PnL$-current": "PnL$-actual",
        "STC-Amount": "STC-Qty",
        "STC-Price-current": "STC-Price-actual",
        "STC-Prices-current": "STC-Prices-actual",
        "STC-PnL-current": "PnL-actual",
        "STC-PnL$-current": "PnL$-actual",
        "STC-PnL$": "PnL$",
        "STC-PnL": "PnL",
    }
...
if not os.path.exists(config_path):
    ...
    print(
        "\033[91mWARNING: Rename DiscordAlertsTrader/config_example.ini to DiscordAlertsTrader/config.ini. \033[0m"
    )
    print(
        "\033[91mWARNING: Reverting to config_example.ini for now (might be necessary for testing). \033[0m"
    )
    ...
    if len(missing_items):
        raise ValueError(
            f"config.ini is missing the following items: {missing_items}\nadd missing items to config.ini and re-run"
        )
...
tracker_portfolio_cols = ",".join(
    ...
    + [
        f"STC-{v}"
        for v in ["Qty", "Price", "Price-actual", "Prices", "Prices-actual", "Date"]
        for i in range(1, 2)
    ]
    + ["TrailStats"]
)
