...
from thetadata import DataType, DateRange, OptionReqType, OptionRight, ThetaClient
...
def get_timestamp_(row):
    date_time = datetime.strptime(str(int(row["date"])), "%Y%m%d") + timedelta(
        milliseconds=row["ms_of_day"]
    )  # ET
...
def ms_to_time(ms_of_day: int) -> datetime.time:
    ...
    return datetime(
        ...
        microsecond=(ms_of_day % 1000) * 1000,
    ).time()
...
class ThetaClientAPI:
    ...
    def info_option(self, symbol: str, date_range: list[date]):
        ...
        expdate = date(
            symb_info["exp_year"], symb_info["exp_month"], symb_info["exp_day"]
        ).strftime("%Y%m%d")
    ...
    def get_hist_trades(self, symbol: str, date_range: list[date], interval_size: int = 1000):
        ...
        agg_funcs = {"bid": "last", "ask": "last", "last": "last", "volume": "sum"}
    ...
    def get_geeks(
        ...
        if fetch_data:
            ...
            merged_df = pd.merge(
                trades[["timestamp", "last", "volume"]], df_q, on="timestamp", how="right"
            )
            data = merged_df[
                [
                    "timestamp",
    ...
    def get_delta_strike2(
        self, ticker: str, exp_date: str, delta: float, right: str, timestamp: int, stock_price=None
    ):
        ...
        """
        # date to ny time
        dt = datetime.fromtimestamp(timestamp, tz=pytz.utc).astimezone(
        ...
        return f"{ticker}_{exp_date_f}{right}{delta_row['strike'] / 1000}".replace(
            ".0", ""
        ), delta_row
    ...
    def get_delta_strike(
        ...
        if stock_price is None:
            ...
            geeks = self.get_geeks(
                f"{ticker}_{expdate}{right}{strikes[mid] / 1000}",
                [dt.date(), dt.date()],
                get_trades=False,
            )
            ...
            strike = find_closest_strike(
                stock_price, dte, right, delta, [s / 1000 for s in strikes]
            )
    ...
    def get_price_at_time(self, symbol: str, unixtime: int, price_type: str = "BTO"):
        ...
        if op.exists(fquote):
            ...
            if unixtime_date in df["date"].values:
                ...
                if df_date["timestamp"].min() <= unixtime <= df_date["timestamp"].max():
                    print(
                        f"{Fore.YELLOW} Found data for {symbol} at {datetime.fromtimestamp(unixtime)}"
                    )
                    fetch_quote = False
                    quotes = df_date
        ...
        print(
            f"{Fore.RED} Error: price for {symbol} not found at {datetime.fromtimestamp(unixtime)}"
        )
        return -1, 0
