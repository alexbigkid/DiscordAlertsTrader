    ...
    for mb in message["embeds"]:
        ...
        if match:
            ...
            if match:
                ...
                alert = (
                    f"{action} {qty} {ticker} {strike.upper()}{otype} {expdate} @{price} {extra}"
                )
        if not match:
            alert = f"{mb['title']}: {mb['description']}"
    return alert
...
def flohai_formatting(message):
    ...
    rating = re.search(r"\*\*AI Confidence Rating:\*\* ([\d.]+)%", info)
    if rating:
        rating = rating.group(1)
    else:
        return alert

    strike = re.search(r"\*\*Strike:\*\* ([\d.]+)", info).group(1)
    expiration = re.search(r"\*\*Expiration:\*\* (\d{1,2}/\d{1,2}/\d{4})", info).group(1)
    ask = re.search(r"\*\*Ask:\*\* ([\d.]+)", info).group(1)
...
def flint_formatting(message):
    ...
    if len(alert):
        pattern = (
            r"([A-Z]+)\s*(\d+[.\d+]*[c|p|C|P])\s(\d{1,2}\/\d{1,2})\s*@\s*(\d+(?:[.]\d+)?|\.\d+)"
        )
        match = re.search(pattern, alert, re.IGNORECASE)
        if match:
            out = match.groups()
...
def eclipse_formatting(message):
    ...
    if match:
        ...
        pattern = (
            r"([A-Z]+)\s*(\d{1,2}\/\d{1,2})?\s*(\d+[.\d+]*[c|p|C|P])\s*@\s*(\d+(?:[.]\d+)?|\.\d+)"
        )
        match = re.search(pattern, alert, re.IGNORECASE)
        if match:
    ...
    for mb in message["embeds"]:
        if mb["title"].replace(":", "") in ["Daytrade", "LOTTO", "Swing"]:
            ...
            contract_match = re.search(
                r"\*\*Contract:\*\* \$([A-Z]+) (\d{1,2}\/\d{1,2}) ([\d.]+)([cCpP])", description
            )
            fill_match = re.search(r"\*\*Entry:\*\*\s*\@?\$?\s*([\d.]+)", description)

            if contract_match is None:
                contract_match = re.search(
                    r"\*\*Ticker: \$([A-Z]+)\*\*.*\*Contract: (\d{1,2}\/\d{1,2})\s+([\d.]+)([cCpP])",
                    description,
                )
                fill_match = re.search(r"\*\*Entry:\s*\@?\$?\s*([\d.]+)\*\*", description)
                if contract_match is None:
            ...
            contract_match = re.search(
                r"\*\*Contract:\*\* \$([A-Z]+) (\d{1,2}\/\d{1,2}) ([\d.]+)([cCpP])", alert
            )
            if contract_match:
                contract, exp_date, strike, otype = contract_match.groups()
    ...
    for msg in data["messages"]:
        ...
        if author in ["demon", "bryce", "moustache"]:
            if msg["content"].startswith("STC") and "@" not in msg["content"]:
                msg["content"] += " @ 1"
            contract = format_0dte_weeklies(
                msg["content"].replace("@Demon alerts", ""), msg_date_ob, False
            )
        ...
        msgs.append(
            [
                msg_date.strftime("%m/%d/%Y %H:%M:%S.%f"),
                msg["author"]["name"],
                content,
                pars,
                msg["content"],
...
if __name__ == "__main__":
    ...
    if author == "demon":
        ...
        fname = r"../DiscordChatExporter.Cli\kent\kent.json"
    elif author == "sirgoldman":
        ...
        for _, row in df.iterrows():
            ...
            date_msg = datetime.strptime(row["Date"][:26], "%Y-%m-%dT%H:%M:%S.%f").strftime(
                "%m/%d/%Y %H:%M:%S.%f"
            )
