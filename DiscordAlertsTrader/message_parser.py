...
def parse_mark_stock(msg, Symbol, act):
    re_mark = re.compile(r"\@[ ]*[$]*[ ]*(\d+(?:\.\d+)?|\.\d+)")
    mark_inf = re_mark.search(msg)
    if mark_inf is None:
        re_mark = re.compile(
            f"{act} " + r"([\*])?([\*])?" + f"{Symbol}" + r"([\*])?([\*])? (\d+(?:\.\d+)?)"
        )
        mark_inf = re_mark.search(msg)
        if mark_inf is None:
            return None
...
def parse_mark_option(msg):
    re_mark = re.compile(r"(?:@|at)[a-zA-Z]?[ ]*[$]?[ ]*([.]?\d+(?:\.\d+)?)")
    mark_inf = re_mark.search(msg)
    if mark_inf is None:
        re_mark = re.compile(r"(?:@|at)[a-zA-Z]?[ ]*[$]?[ ]*([,]?\d+(?:\.\d+)?)")
        mark_inf = re_mark.search(msg)
    ...
    if mark_inf is None:
        date = parse_date(msg)
        re_mark = re.compile(rf"{date}[ ]*[$]?[ ]*([.]?\d+(?:\.\d+)?)")
        mark_inf = re_mark.search(msg)
        if mark_inf is None:
            return None
...
def parse_strike(msg):
    re_strike = re.compile(r" [$]?(\d+(?:\.\d+)?)(C|c|P|p)")
    ...
    if strike_inf is None:
        ...
        re_strike = re.compile(rf"{sym} (\d+(?:\.\d+)?)")
        strike_inf = re_strike.search(msg)
        if strike_inf is None:
            return None, None
...
def parse_date(msg):
    ...
    re_date = re.compile(r"(\d{1,2}\/\d{1,2}(?:\/\d{1,2})?)")
    date_inf = re_date.search(msg)
    if date_inf is None:
        months = [
            "Jan",
        ...
        exp = f"({'|'.join(months)})" + r" (\d{1,2}) (20\d{2})"
...
def parse_avg(msg):
    re_avg = re.compile(r"(?:avg[.]?|new average)[ ]*[$]*(\d+(?:\.\d+)?)", re.IGNORECASE)
...
def parse_exits_vals(msg, expr):
    re_comp = re.compile(
        r"\s" + expr + r"[:]?[ ]*[$]*(\d*[\.]*[\d]*[%]?)(TS[\d+\.]*[%]?)?", re.IGNORECASE
    )
    exit_inf = re_comp.search(msg)

    if exit_inf is None:
        re_comp = re.compile(
            r"(\s" + expr.lower() + r"[:]?[ ]*[$]*(\d*[\.]*[\d]*[%]?))", re.IGNORECASE
        )
        exit_inf = re_comp.search(msg)
...
def parse_sell_ratio_amount(msg, asset):
    ...
    exprs = r"(?:sold|sell) (\d\/\d)"
    re_comp = re.compile(exprs, re.IGNORECASE)
    amnt_inf = re_comp.search(msg)
    if amnt_inf is not None:
        return round(eval(amnt_inf.groups()[0]), 2)

    exprs = r"(?:sold|sell)(\d of \d)"
    re_comp = re.compile(exprs, re.IGNORECASE)
    amnt_inf = re_comp.search(msg)
    if amnt_inf is not None:
        return round(eval(amnt_inf.groups()[0].replace(" of ", "/")), 2)

    exprs = r"(?:sold|sell) (\d{1,2})%"
    re_comp = re.compile(exprs, re.IGNORECASE)
    amnt_inf = re_comp.search(msg)
    if amnt_inf is not None:
