...
class AlertsTrader:
    def __init__(
        ...
        if update_portfolio:
            ...
            self.queue_prints.put(
                [f"Updating portfolio orders every {self.order_update_rate} secs", "", "green"]
            )
    ...
    def order_to_pars(self, order):
        ...
        if order["action"] in ["BTO", "STO"]:
            for i in range(1, self.max_stc_orders):
                pt = f"PT{i}"
                if pt in order and order[pt] is not None:
                    pars_str = pars_str + f" {pt}: {order[pt]}"
                if "SL" in order and order["SL"] is not None:
    ...
    def confirm_and_send(self, order, pars, order_funct):
        ...
        if resp in ["yes", "y"]:
            ...
            str_msg = (
                f"Sent order {order['action']} {order['Qty']} {order['Symbol']} @{order['price']}"
            )
            print(Back.GREEN + str_msg)
    ...
    def short_orders(self, order, pars):
        if order["action"] == "STO":
            ...
            if sto_price == "alert":
                check_price = False
            ptype = (
                "BTO" if sto_price in ["ask", "mid"] else "last" if sto_price == "last" else "STO"
            )
            if check_price:
            ...
            if order.get("Qty") is not None and not self.cfg["shorting"].getboolean(
                ...
                if order["asset"] == "option":
                    ...
                    order["Qty"] = int(
                        max(float(self.cfg["shorting"]["trade_capital"]) // order["price"], 1)
                    )
        ...
        while True:
            ...
            if self.cfg["order_configs"].getboolean("auto_trade") is True:
                if (
                    ...
                    str_msg = "BTO not accepted by config options: DO_BTO_TRADES = False"
                    print(Back.GREEN + str_msg)
                    self.queue_prints.put([str_msg, "", "green"])
                    return "no", order, False
                    ...
                    if len(cfg["order_configs"]["exclude_tickers"]):
                        ...
                        if order["Symbol"].split("_")[0] in no_trade:
                            str_msg = (
                                f"BTO not accepted by config options: exclude_tickers = {no_trade}"
                            )
                            print(Back.GREEN + str_msg)
                            self.queue_prints.put([str_msg, "", "green"])
                            return "no", order, False
                    ...
                    price = order["price"]
                    if price == 0:
                        str_msg = "Order not accepted price is 0"
                        print(Back.GREEN + str_msg)
                        self.queue_prints.put([str_msg, "", "red"])
                        return "no", order, False
                    ...
                    if "Qty" not in order or order["Qty"] is None:
            ...
            if resp in ["c", "change", "y", "yes"] and "Qty" not in order:
                order["Qty"] = int(
                    input(
        ...
        if order["action"] == "ExitUpdate" and isOpen:
            ...
            if self.portfolio.loc[open_trade, "Asset"] == "option":
                ...
                for i in range(1, self.max_stc_orders):
                    ...
                    if new_plan.get("SL"):
                        new_plan["SL"] = set_exit_price_type(new_plan.get("SL"), new_plan)
            ...
            for i in range(1, 3):
                if not pd.isnull(self.portfolio.loc[open_trade, f"STC{i}-alerted"]):
                    istc = i + 1
            if istc is not None and any("PT" in k for k in new_plan):
            ...
            self.queue_prints.put(
                [f"Updated {symb} exit plan from :{old_plan} to {renew_plan}", "", "green"]
            )
            return
            ...
            self.portfolio = pd.concat(
                [self.portfolio, pd.DataFrame.from_records(new_trade, index=[0])], ignore_index=True
            )
            ...
            if open_trade is None:
                ...
                self.save_logs()
                if (
                    self.cfg["general"].getboolean("DO_BTO_TRADES") and order["action"] == "STC"
                ) or (self.cfg["general"].getboolean("DO_STO_TRADES") and order["action"] == "BTC"):
                    print(Back.GREEN + str_msg)
                    self.queue_prints.put([str_msg, "", "green"])
                return
            ...
            if order.get("amnt_left"):
                ...
                print(
                    Back.GREEN
                    + "Based on alerted amnt left, Updated order: "
            ...
            if qty_bought == 0 and order["xQty"] == 1:
                ...
                self.queue_prints.put(
                    [f"Order Cancelled {order['Symbol']}, closed before fill", "", "green"]
                )
    ...
    def log_filled_STC(self, order_id, open_trade, STC):
        ...
        stc_PnL_all = (
            np.nansum(
                [trade[f"STC{i}-PnL"] * trade[f"STC{i}-Qty"] for i in range(1, self.max_stc_orders)]
            )
            / sold_tot
        )
            ...
            stc_PnL_all_alert = (
                np.nansum(
                    [
                        (
                            float(
                                (trade[f"STC{i}-Price-alert"] - bto_price_alert) / bto_price_alert
                            )
                            * 100
                        )
                        * trade[f"STC{i}-Qty"]
            ...
            stc_PnL_all_alert = (
                np.nansum(
                    [
                        (
                            float(
                                (bto_price_alert - trade[f"STC{i}-Price-alert"]) / bto_price_alert
                            )
                            * 100
                        )
                        * trade[f"STC{i}-Qty"]
        ...
        for i in range(len(self.portfolio)):
            ...
            if trade["BTO-Status"] == "invTSbuy":
                ...
                if quote_opt <= stp_price * 1.01:  # 1%
                    qty = 1 if pd.isna(trade["trader_qty"]) else int(trade["trader_qty"])
                    order = {
                        "Symbol": trade["Symbol"],
                        "action": "BTO",
                        "asset": trade["Asset"],
            ...
            if trade["BTO-Status"] not in ["FILLED", "CANCELED", "REJECTED"]:
                ...
                # If so, redo orders
                if not (pd.isnull(qty_fill_old) or qty_fill_old == 0) and qty_fill_old != qty_fill:
                    redo_orders = True
            ...
            if not pd.isnull(trade.get("BTO-avg-Status")) and trade.get("BTO-avg-Status") not in [
                "FILLED",
                "CANCELED",
                "REJECTED",
                "MISSING",
            ...
            if trade["Type"] == "STO" and (
                ...
                for avg in avg_down_list:
                    ...
                    if avg_price < price_currernt and avg_down.get(str(avg_rat)) is None:
                        ...
                        if order_status in ["FILLED", "EXECUTED"]:
                            or_price = (
                                self.portfolio.loc[i, "Price"] * self.portfolio.loc[i, "filledQty"]
                            )
                            nw_price = order_info["price"] * order_info["filledQuantity"]
                            avg_price = round(
                                (or_price + nw_price)
    ...
    def make_exit_orders(self, open_trade, exit_plan):
        ...
        nPTs = len(
            [i for i in range(1, self.max_stc_orders) if exit_plan.get(f"PT{i}") is not None]
        )
        if nPTs != 0:
    ...
    def calculate_stoploss(self, order, trade, SL: str):
        ...
        if isinstance(SL, str):
            SL = trade["Price"] * float(SL.replace("%", "")) / 100 if "%" in SL else float(SL)
    ...
    def round_price(self, price, trade):
        ...
        if self.bksession.name in ["tda"]:
            increment = (0.05 if price < 3.0 else 0.1) if "SPXW" in trade["Symbol"] else 0.01
        elif self.bksession.name == "ts":
            if "SPXW" in trade["Symbol"]:
                increment = 0.05 if price < 3.0 else 0.1
            elif price > 3:
                increment = 0.05
            else:
                increment = 0.01
