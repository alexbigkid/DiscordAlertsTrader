...
class TDA(BaseBroker):
    ...
    def get_session(
        ...
        if len(cfg["TDA"]["client_id"]) < 10:
            raise ValueError(
                "No TDA authentication file found, get credentials (in setup.py) to continue"
            )
    ...
    def send_order(self, new_order):
        order_response = self.session.place_order(account=self.accountId, order=new_order)
    ...
    def make_BTO_lim_order(
        self, Symbol: str, Qty: int, price: float, strike=None, action="BTO", **kwarg
    ):
    ...
    def make_STC_SL_trailstop(
        self, Symbol: str, Qty: int, trail_stop_const: float, new_order=None, action="STC", **kwarg
    ):
        if new_order is None:
            new_order = Order()
        new_order.order_strategy_type("SINGLE")
