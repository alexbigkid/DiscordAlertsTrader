...
class TS(BaseBroker):
    ...
    def get_session(self):
        ...
        self.session = tsa.easy_client(
            cfg["tradestation"]["client_id"],
        ...
        if self.accountId is None:
            ...
            self.accountId = [
                k["Name"] for k in resp if k["TypeDescription"] == cfg["tradestation"]["acct_type"]
            ][0]
    ...
    def get_order_info(self, order_id):
        ...
        if (
            ...
            and order_info["Message"] == "No orders were found for the specified Order IDs."
        ):
            print(order_info)
        ...
        if len(order_info["Orders"]) > 1:  # bracket orders
            ix = [
                i
                for i, order in enumerate(order_info["Orders"])
                if order["StatusDescription"].upper() == "FILLED"
            ]
    ...
    def get_positions_orders(self):
        ...
        for pos in positions["Positions"]:
            ...
            df_pos = pd.concat(
                [df_pos, pd.DataFrame.from_records(pos_inf, index=[0])], ignore_index=True
            )

        df_ordr = pd.DataFrame(columns=["symbol", "asset", "type", "Qty", "Price", "action"])
    ...
    def fix_order(self, new_order, resp):
        ...
        # Iterate over resp['Orders'] and check if any message matches the pattern
            ...
            for order_ in orders["Orders"]:
                ...
                symbol_neworder = (
                    self._convert_option_fromts(new_order["Symbol"])
                    if new_order.get("Symbol")
                    else self._convert_option_fromts(new_order["Orders"][0]["Symbol"])
                )
                symbol_anyorder = (
                    self._convert_option_fromts(order["symbol"])
                    if order.get("symbol")
            ...
            new_qty = int(
                re.search(r"Order failed. Reason: EC702: You are \w+ ([\d.]+) shares!", text)
                .group(1)
                .replace(".00", "")
            )
