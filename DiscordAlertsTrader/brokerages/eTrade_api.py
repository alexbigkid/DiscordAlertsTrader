...
class eTrade(BaseBroker):
    ...
    def _get_account(self):
        ...
        if self.accountId is not None:
            self.accountIdKey = [
                self.accounts_list[i]["accountIdKey"]
                for i in range(len(self.accounts_list))
                if self.accounts_list[i]["accountId"] == self.accountId
            ][0]
        else:
    ...
    def get_account_info(self):
        ...
        acc_inf = {
            "securitiesAccount": {
                ...
                "currentBalances": {
                    "liquidationValue": data["BalanceResponse"]
                    .get("Computed")
                    .get("RealTimeValues")
                    .get("totalAccountValue"),
                    "cashBalance": data["BalanceResponse"].get("Computed").get("cashBalance"),
                    "availableFunds": data["BalanceResponse"]
                    .get("Computed")
                    .get("cashAvailableForInvestment"),
                },
            }
        }
            ...
            for acctPortfolio in data["PortfolioResponse"]["AccountPortfolio"]:
                if acctPortfolio is not None and "Position" in acctPortfolio:
                    for position in acctPortfolio["Position"]:
                        assetType = (
                            position["Product"]["securityType"]
                            .replace("EQ", "stock")
                            .replace("OPTN", "OPTION")
                        )
                        pos = {
                            "longQuantity": position["quantity"]
                ...
                if "QuoteData" in data["QuoteResponse"]:
                    for quote in data["QuoteResponse"]["QuoteData"]:
                        ...
                        if quote.get("Product").get("securityType") == "OPTN":
                            ...
                            prod = quote["Product"]
                            opty = (
                                prod["callPut"][0].upper().replace("CALL", "C").replace("PUT", "P")
                            )
                        ...
                        if resp[ticker]["status"].upper() == "DELAYED":
                            print(
                                "\033[91mWARNING: QUOTES ARE DELAYED by 15 min, setup realtime quotes in etrade.com. Info in github README.md \033[0m"
                            )
        return resp
    ...
    def get_order_info(self, order_id):
        ...
        for order in orders["OrdersResponse"]["Order"]:
            if order["orderId"] == order_id:
                order_status = (
                    order["OrderDetail"][0]["status"]
                    .upper()
                    .replace("EXECUTED", "FILLED")
                    .replace("OPEN", "WORKING")
                )
    ...
    def send_order(self, new_order: dict):
        order_response = self.order_session.place_equity_order(
            resp_format="xml", accountId=self.accountId, accountIdKey=self.accountIdKey, **new_order
        )
        ...
        order_response["quantity"] = (
            int(order_response["PlaceOrderResponse"]["Order"]["Instrument"]["quantity"]),
        )
