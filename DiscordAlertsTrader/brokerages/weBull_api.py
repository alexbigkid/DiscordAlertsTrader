...
from DiscordAlertsTrader.brokerages.webull import paper_webull, webull
...
class weBull:
    ...
    def get_session(self, use_workaround: bool = True) -> bool:
        ...
        wb.login(
            username=cfg["webull"]["LOGIN_EMAIL"],
        ...
        self.session = wb
        if self.success:
            print("Logging into webull: Success!")
        else:
            print("Logging into webull: Failed!")
        return self.success
    ...
    def get_account_info(self):
        ...
        for position in positions:
            ...
            pos = {
                ...
                "currentDayProfitLossPercentage": float(eval(position["unrealizedProfitLoss"]))
                / 100,
                "instrument": {
                    "symbol": position["ticker"]["symbol"],
                    "assetType": assetType,
        ...
        if isinstance(orders, dict) and orders.get("success") is False:
            raise ValueError(
                "Order entpoint obsolete, go to webull/endpoints.py (actual webull package) line 144 and remove '&startTime=1970-0-1'"
            )
    ...
    def format_order(self, order: dict):
        ...
        if asset == "option":
            ...
            symbol = f"{order['orders'][0]['symbol']}_{mnt}{day}{yer[2:]}{otype}{order['orders'][0]['optionExercisePrice']}".replace(
                ".00", ""
            )
    ...
    def get_option_id(self, symb: str):
        ...
        if self.option_ids.get(symb) is None:
            ...
            if opt_info:
                options_data = self.session.get_options(
                    stock=opt_info["ticker"],
                ...
                filtered_options_data = [
                    ...
                    if option["strikePrice"] == opt_info["strike"]
                    and option[opt_info["direction"]]["expireDate"] == opt_info["date"]
                ]
            ...
            if "_" in symb:
                ...
                if opt_info:
                    try:
                        ...
                        quote = self.session.get_option_quote(
                            stock=opt_info["ticker"], optionId=option_id
                        )
                        ...
                        resp[sym_out] = {"symbol": sym_out, "description": "Symbol not found"}
            else:
                ...
                if quote and quote["template"] == "stock":
                    ...
                    resp[symb] = {"symbol": symb, "description": "Symbol not found"}
        return resp
    ...
    def make_Lim_SL_order(self, Symbol: str, Qty: int, PT: float, SL: None, action="STC", **kwarg):
        ...
        if SL is not None:
            print(
                "WARNING: webull api does not support OCO, setting a SL only, do not provide SL to send PT"
            )
