...
class SW(BaseBroker):
    ...
    def get_session(self):
        ...
        if not os.path.exists("schwab_token.json"):
            ...
            self.session = schwab.auth.client_from_manual_flow(
                cfg["schwab"]["key"],
                cfg["schwab"]["secret"],
            ...
            self.session = schwab.auth.client_from_token_file(
                "schwab_token.json", cfg["schwab"]["key"], cfg["schwab"]["secret"]
            )
    ...
    def _convert_option_fromsw(self, ticker):
        ...
        return f"{symb}_{formatted_date}{right}{strike}".replace(
            ".0", ""
