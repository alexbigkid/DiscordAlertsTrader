...
class AlertsTracker:
    def __init__(
        ...
        if op.exists(self.portfolio_fname):
            ...
            self.portfolio = pd.DataFrame(
                columns=self.cfg["col_names"]["tracker_portfolio"].split(",")
            )
    ...
    def price_now(self, symbol: str, price_type="BTO"):
        ...
        if (
            quote is not None
            and len(quote)
            and quote.get(symbol) is not None
            and quote.get(symbol).get("description") != "Symbol not found"
        ):
    ...
    def trade_alert(self, order, live_alert=True, channel=None):
        ...
        if (
            order["action"] in ["BTO", "STC", "STO", "BTC"]
            and live_alert
            and order.get("Actual Cost", "None") == "None"
        ):
    ...
    def make_BTO(self, order, chan=None):
        ...
        new_trade = {
            ...
            "SL": order.get("SL"),
            "Channel": chan,
            "underlying": option_info.get("strike"),
        }
    ...
    def make_BTO_Avg(self, order, open_trade):
        ...
        alert_price_old = (
            eval(alert_price_old) if isinstance(alert_price_old, str) else alert_price_old
        )
        alert_price_old = None if pd.isnull(alert_price_old) else alert_price_old
        alert_price = order.get("Actual Cost", "None")
        avgs_prices_al = (
            f"{alert_price_old}/{alert_price}".replace("None/", "")
            .replace("/None", "")
            .replace("None", "")
        )
        ...
        self.portfolio.loc[open_trade, "Price"] = round(
            ((old_price * old_qty) + (order["price"] * order["Qty"])) / (old_qty + order["Qty"]), 2
        )
        ...
        if alert_price == "None" or alert_price is None or alert_price_old is None:
            ...
            self.portfolio.loc[open_trade, "Price-actual"] = round(
                ((alert_price_old * old_qty) + (alert_price * order["Qty"]))
                / (old_qty + order["Qty"]),
                2,
            )
        ...
        str_act = (
            f"{order['action']} {order['Symbol']} {actual_Avg}th averging down @ {order['price']}"
        )
        return str_act
    ...
    def make_STC(self, order, open_trade, check_trail=False):
        ...
        if stc_price == "none" or stc_price is None:
            str_STC = (
                f"{order['action']} {order['Symbol']}  ({order['Qty']}), no price provided" + suffx
            )
        else:
    ...
    def compute_trail(self, open_trade):
        ...
        res_str = "TS:{},{}%,${},in {}"
        trailing_Stop = [0.2, 0.3, 0.4, 0.5]
        max_trails = []
        quotes["highest"] = quotes[" quote"].cummax()  # take the cumulative max
        quotes["perc"] = round(
            (quotes[" quote"] - quotes[" quote"].loc[0]) / quotes[" quote"].loc[0] * 100, 2
        )
        ...
        quotes_stats = "| "
        for st, ix in zip(
            ["min", "max"], [quotes["perc"].idxmin(), quotes["perc"].idxmax()], strict=False
        ):
    ...
    def close_expired(self):
        for i, trade in self.portfolio.iterrows():
            ...
            if optdate.date() < date.today():
                expdate = pd.to_datetime(optdate).replace(
...
def calc_stc_prices(trade, order=None):
    ...
    if order is None:
        order = {"price": 0, "Actual Cost": 0, "expired": True}
